...
- **Taxe d'aménagement €/m²** (ParaGEN!B31:B32) : 930 hors IdF, 1054 IdF. ⚠️ Ce sont les valeurs **2025**, celles du profil précédent comme tout ce paragraphe. Le référentiel en vigueur porte **892 / 1 011** au 1er janvier 2026 : ces valeurs sont actualisées chaque année sur l'ICC et **baissent** en 2026. Une valeur en repli n'est donc pas le signe d'une erreur de relevé - c'est ce que l'audit du 03/09/2026 a d'abord cru, à tort.
...
- **R-LOYER-3** ✅ Marges locales de majoration : marge = MIN(Σ majorations affectées, plafond ParaPLUS!AD30), plafonnement séparé PLUS/PLAI. Source FinPLUS!S26:T29. **Appliqué depuis le 03/09/2026** : la règle était déclarée et le plafond éditable à l'écran, mais `margePlafonnee` n'était appelée par personne. Une marge de 200 % triplait le loyer d'un PLUS sans un mot, sur un loyer qu'aucune convention n'aurait accepté. Le plafonnement lève désormais une alerte, et il ne s'applique **pas** à un produit à loyer de marché (`loyer_de_marche`), où la marge est le pari commercial lui-même et non un dépassement de convention.
...
- **R-SUB-3** ✅ Gratuité et rattachement : chaque subvention porte un flag gratuit (1/0) et la tranche qui la porte. Agrégats gratuites/non gratuites → équilibre. Source calculs!D295:D314. **Une subvention est rattachée à un financement, comme un prêt ou des fonds propres, ou elle n'existe pas** (arbitrage métier du 10/09/2026). Trois cas : la tranche nommée existe au programme et porte la subvention ; aucune tranche nommée mais une seule au programme, qui la prend sans ambiguïté (le moteur rattache de même un prêt sans tranche) ; sinon - aucune tranche nommée sur un programme mixte, ou une tranche qui n'y figure plus - la subvention reste **hors plan** : elle ne finance rien, ne compte dans aucun total, une alerte la nomme, et le plan de financement la présente pour qu'on la rattache ou qu'on la supprime. LEON répartit une subvention sans affectation au prorata des surfaces utiles (écart E-14) : la répartition fabriquait des centimes que chaque tranche arrondissait de son côté, et c'est ainsi que la démonstration Îlot Verrières sortait déséquilibrée d'un euro. Le PLUS et le PLAI sont deux tranches distinctes : le couple « PLUS-PLAI » de LEON n'existe pas ici (écart E-13), une aide qui vise les deux se saisit en deux lignes. Supprimer une tranche emporte ses subventions, comme ses prêts. Une subvention rattachée au libre n'est pas refusée - une participation de collectivité de droit commun existe - mais elle est signalée. Seule la subvention de surcharge foncière, calculée et non saisie, se répartit encore au prorata des surfaces, en euros entiers dont la somme conserve son montant.
...
- **R-EXP-RLS** ✅ **Réduction de loyer de solidarité** (03/09/2026, CCH art. L. 442-2-1). Le bailleur social diminue le loyer de ses locataires bénéficiaires de l'APL. Ce n'est **ni une charge ni une vacance** : c'est du loyer **quittancé en moins**, et il se retire donc avant tout ce qui s'assoit sur les loyers - vacance, frais de gestion au pourcentage, cotisations CGLLS et ANCOLS. Le retirer plus loin, en charge, aurait donné le même résultat net mais faussé toutes les assiettes en chemin. Le taux ne porte que les **loyers de logements** : ni les annexes louées séparément, ni une redevance de foyer ne sont quittancées au sens de la RLS. Décidée **tranche par tranche**, comme l'impôt sur les sociétés et pour la raison inverse : le parc conventionné APL la subit, le logement intermédiaire et le libre non. **Nulle par défaut** - le taux du référentiel (4 % du quittancement, note de cadrage PMT 2026-2035) est une *projection*, pas un barème arrêté, et l'activer d'office aurait déplacé en silence le résultat de toutes les simulations enregistrées. Référentiel `charges_exploitation.reduction_loyer_solidarite` ; la saisie l'active par `exploitation.rls_actif` ou en posant un taux. Reste ouvert : Q-43.
...
- **R-FISC-2** ✅ Taxe d'aménagement : assiette = SDP × (1 − abattement) × valeur_forfaitaire + parkings ×… Source calculs!B1255:B1259. **Régime par PRODUIT depuis le 03/09/2026** : l'abattement n'est pas uniforme. Le PLAI (et le LLTS outre-mer) ouvre une **exonération de plein droit**, CGI art. 1635 quater D, I, 2° ; les autres logements financés par un prêt aidé de l'État - PLUS, PLS, foyers - n'ont que l'**abattement de 50 %**, art. 1635 quater I, l'exonération primant sur l'abattement ; le LLI et le libre n'ont **ni l'un ni l'autre**, faute de prêt aidé. Un abattement de 50 % appliqué à tout surtaxait donc le PLAI et sous-taxait le libre sur la même opération. L'assiette se ventile à la quote-part de SDP de chaque tranche (à défaut, la quote-part SU) et chacune applique son régime ; un abattement saisi force la valeur pour toute l'opération, recours quand une délibération locale s'écarte du droit commun (art. 1635 quater E). Valeurs forfaitaires **892 / 1 011 €/m² au 1er janvier 2026** (art. 1635 quater H), vérifiées le 03/09/2026 : elles sont actualisées chaque année sur l'ICC et **baissent** en 2026 après 930 / 1 054 en 2025 - une valeur en repli n'est donc pas le signe d'une erreur de relevé.
- **R-FISC-3** ❌ **RETIRÉE le 03/09/2026 : le versement pour sous-densité est ABROGÉ.** Loi n° 2020-1721 du 29 décembre 2020 (loi de finances pour 2021), article 155 : le versement disparaît au 1er janvier 2021, plus aucun seuil minimal de densité ne peut être institué et ceux qui existaient cessent de produire effet. Le code était écrit et testé, et n'a jamais été appelé par le moteur : il modélisait un impôt qui n'existe plus. Trace en pied de `src/fiscalite.js` ; les valeurs restent au référentiel sous `abroge: true`, pour qu'une simulation ancienne qui les porte reste lisible. Ancienne formule, pour mémoire : si neuf et seuil densité > 0, MIN(valeur_terrain/2 × (1 − SDP/(seuil×surface)), 25 % × valeur_terrain).
...
## 12 bis. Règles écrites mais NON BRANCHÉES (audit du 03/09/2026)

Trois fonctions du moteur sont implémentées et testées unitairement, mais **aucun chemin de `calculer` ne les appelle**. Elles ne produisent donc rien sur une simulation réelle. Ce n'est pas un défaut - ce sont des règles dont le métier n'a pas encore fixé les entrées - mais leur statut ✅ au dictionnaire le laissait croire, et l'audit l'a relevé. Le statut est corrigé ici plutôt que dans chaque section.

| Règle | Fonction | Ce qui manque pour la brancher |
|---|---|---|
| R-SUB-1 (SLA) | `subventionEtat` | Le mode et le forfait. En métropole l'assiette réglementaire est nulle et la subvention vient d'un forfait négocié : tant qu'il n'est pas une saisie de l'écran, la subvention d'État se pose comme une ligne de subvention ordinaire, ce qui donne le même montant. |
| R-LOYER-4 (LCR) | `majorationLCR` | Le ratio de surfaces LCR, qui n'est pas au modèle d'entrée. La borne intermédiaire reste à confirmer (Q-10). |
| R-LOYER-6 | *(non écrite)* | Annexe importante : transcription fine jamais faite, cas de test dédié à construire. |

En revanche `margePlafonnee` (R-LOYER-3) et la table `valeurs_de_base` (R-SUB-2) **ont été branchées** le 03/09/2026 : elles étaient dans le même état, et leur absence, elle, faussait des montants.
