...
// `presets` : la section des prets. L'ancien identifiant `prets` n'existait
// plus depuis la fusion des marges CDC et des modeles, et l'ecran s'ouvrait
// sur une page vide jusqu'au premier clic dans le rail.
let sectionParametres = 'presets';
...
function rendreParametres() {
  ...
  if (enRecherche) {
    ...
    const affichees = sections.filter(
      (s) => s.id === sectionParametres && s.id !== 'hypotheses',
    );
